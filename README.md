...
- **[Spellbound](spellbound/)**: a game that trains you to find chords and notes on the minichord quickly, with answers timed. Read and play: Spell it, Numerals, On the staff (engraved in Bravura), Slash chords, Key detective, and a mix. The harp, in "Scale per chord" so the chord you hold sets the scale: Harp hunt, Play by number (tunes in scale degrees), Simon says (a melody that grows by a note each round) and Missing note, plus Chord scales, where the game writes a real scale into the custom scale on the chord you press and you name it. Scale rounds can be answered on the chord buttons: a mode by pressing the major chord whose scale has the same notes, anything else through a keypad lettered C to B. The chromatic harp (address 98): Follow the directions (up a fifth, down a minor third…), Pluck the chord and Build the scale. Voice leading: Smooth moves, checked against the minichord's own voice-leading search. Mystery settings: Scale detective, Transpose detective, Temperament taster and Tune up. Scales come in four levels, everyday to exotic (phrygian dominant, Hungarian minor, the Neapolitans, altered, Persian, enigmatic and more). Every game but Temperament taster runs the minichord in equal temperament, and each round sets what it needs (harp mode, chromatic mode, MPE, voice leading, slash voice) by itself; all of it goes back when you leave the game that needed it, or at "Put my settings back". Every answer says how to press it on your minichord, and every game works without one through pickers and the piano.
- **[Choir](choir/)**: each voice of the chord sung by its own singer, from recorded choirs or synthesized ones, with the minichord's MPE output, voice leading, glide and temperament switched from the page, a pitch trace, a voice-motion readout, an MPE MIDI recorder, a demo of the minichord's own voicings with voice leading off and on, and slash chords: on firmware 13, "slash voice" (bass, tenor, alto or soprano, counted up from the bottom) and "re-voice", with the singing voice's strip marked and each slash drawn on the trace; on older firmware, "slash replaces". It keeps its own chant-book look.

## Layout

- `core/minichord.js`: the shared connection to the minichord. It reads the chord port over Web MIDI with MPE zones and per-voice bends, so every voice has an exact pitch, and reads the parameter dump over sysex for the key, temperament and MPE settings. Pages listen for `voices`, `chord`, `device`, `status` and `harp` events; `harp` reports each plucked string, from the harp port or the harp channels in single port mode. A `chord` event waits for any glide to finish, since the minichord sends each new note first and then bends the voice home, so views always see the chord as it lands.
...
- `core/music.css` and `core/fonts/`: subsets of Steinberg's Bravura for music: the clef, whole notehead and accidentals for engraved staves (`lab-music.woff2`), and the text accidentals ♭ ♮ ♯ 𝄫 𝄪 (`lab-accidentals.woff2`, from Bravura Text), which `core/lab.css` puts first in every font stack so chord symbols get real music signs. Both are under the SIL Open Font License and renamed as its Reserved Font Name requires; the licence is in `core/fonts/OFL-lab-music.txt`.
