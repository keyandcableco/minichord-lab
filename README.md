...
- `core/minichord.js`: the shared connection to the minichord. It reads the chord port over Web MIDI with MPE zones and per-voice bends, so every voice has an exact pitch, and reads the parameter dump over sysex for the key, temperament and MPE settings. Pages listen for `voices`, `chord`, `device` and `status` events. A `chord` event waits for any glide to finish, since the minichord sends each new note first and then bends the voice home, so views always see the chord as it lands.
